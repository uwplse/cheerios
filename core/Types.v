...
Module Type SERIALIZERCLASS (Writer : WRITER) (Reader : READER).
  ...
  Parameter serialize_deserialize_id_nil : forall A (sA : Serializer A) a,
      Reader.unwrap deserialize (Writer.unwrap (serialize a)) = Some (a, []).
  Parameter serialize_top : forall {A: Type} {sA : Serializer A},
      (A -> Writer.t) -> A -> Writer.wire.
  Parameter deserialize_top : forall {A: Type} {sA: Serializer A},
      Reader.t A -> Writer.wire -> option A.
  Parameter serialize_deserialize_top_id : forall {A : Type} {sA: Serializer A} a,
